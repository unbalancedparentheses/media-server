...
configure_qbittorrent() {
  ...
  if [ -n "$QBIT_COOKIE" ]; then
    ...
    local admin_net=""
    [ "$(admin_host)" != "127.0.0.1" ] && admin_net="$(admin_host)/32"
    QBIT_PREFS=$(jq -nc \
...
configure_sabnzbd() {
  if [ -n "$SABNZBD_KEY" ]; then
    ...
    curl -sf "$SABNZBD_URL/api?mode=set_config&section=misc&keyword=complete_dir&value=$(urlencode "$DOWNLOADS_DIR/usenet/complete")&apikey=$SABNZBD_KEY&output=json" >/dev/null 2>&1
    curl -sf "$SABNZBD_URL/api?mode=set_config&section=misc&keyword=download_dir&value=$(urlencode "$DOWNLOADS_DIR/usenet/incomplete")&apikey=$SABNZBD_KEY&output=json" >/dev/null 2>&1
    ok "Directories: $DOWNLOADS_DIR/usenet/{complete,incomplete}"
